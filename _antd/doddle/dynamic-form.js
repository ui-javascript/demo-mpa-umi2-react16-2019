...
class Basic extends React.Component {
  constructor(props){
    super(props);
    this.state = { reduceRules: [] };
  }

  componentDidMount() {
    setTimeout(() => {
      this.setState({ reduceRules: [{ full: 80, reduction: 5 }, { full: 50, reduction: 3 }] });
    }, 200);
  }

  render() {
    const { reduceRules } = this.state;
    return (
      <div className="p-5">
        <Form>
          <Row>
            ...
            <Col span={18} offset={6}>
              <FormItem {...formItemLayout} label="满减规则">
                <DynamicForm key="fullRules" value={reduceRules} canMove >
                  {(rule, dataBind) => <span key={rule.key}>
                  <span>满</span>
                  <InputNumber
